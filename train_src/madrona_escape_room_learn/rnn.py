...
class LSTM(nn.Module):
    ...
    def lstm_iter_slow(self, layer_idx, in_features, cur_hidden, breaks):
        weight_ih, bias_ih, weight_hh, bias_hh = self._get_lstm_params(layer_idx)

        ifgo = F.linear(in_features, weight_ih, bias_ih) + F.linear(
            cur_hidden[0, :, :], weight_hh, bias_hh
        )

        hs = self.hidden_shape[-1]  # hidden feature size

        c = F.sigmoid(ifgo[:, hs : 2 * hs]) * cur_hidden[1, :, :] + F.sigmoid(
            ifgo[:, 0:hs]
        ) * F.tanh(ifgo[:, 2 * hs : 3 * hs])
    ...
    def fwd_sequence_slow(self, in_sequences, start_hidden, sequence_breaks):
        ...
        zero_hidden = torch.zeros(
            (2, self.num_layers, 1, hidden_dim_per_layer),
        ...
        for i in range(seq_len):
            ...
            cur_hidden = torch.where(cur_breaks.bool(), zero_hidden, cur_hidden)
    ...
    def fwd_sequence_default(self, in_sequences, start_hidden, sequence_breaks):
        ...
        for i in range(seq_len):
            ...
            cur_hidden = torch.where(cur_breaks.bool(), zero_hidden, new_hidden)
