...
class DiscreteActor(nn.Module):
    ...
    def forward(self, features_in):
        ...
        return DiscreteActionDistributions(self.actions_num_buckets, logits=logits)
...
class ActorCritic(nn.Module):
    ...
    def forward(self, rnn_states, *obs):
        actor_features, critic_features, new_rnn_states = self.backbone(rnn_states, *obs)
    ...
    def fwd_actor(self, actions_out, rnn_states_out, rnn_states_in, *obs_in):
        actor_features = self.backbone.fwd_actor_only(rnn_states_out, rnn_states_in, *obs_in)

        action_dists = self.actor(actor_features)
        action_dists.best(out=actions_out)

    def fwd_critic(self, values_out, rnn_states_out, rnn_states_in, *obs_in):
        features = self.backbone.fwd_critic_only(rnn_states_out, rnn_states_in, *obs_in)
    ...
    def fwd_update(self, rnn_states, sequence_breaks, rollout_actions, *obs):
        ...
        flattened_actions = rollout_actions.view(T * N, *rollout_actions.shape[2:])
...
class BackboneEncoder(nn.Module):
    ...
    def fwd_sequence(self, rnn_start_states, sequence_breaks, *flattened_inputs):
...
class RecurrentBackboneEncoder(nn.Module):
    ...
    def fwd_sequence(self, rnn_start_states, sequence_breaks, *flattened_inputs):
        ...
        features_seq = features.view(*sequence_breaks.shape[0:2], *features.shape[1:])

        with profile("rnn.fwd_sequence"):
            rnn_out_seq = self.rnn.fwd_sequence(features_seq, rnn_start_states, sequence_breaks)
...
class BackboneShared(Backbone):
    def __init__(self, process_obs, encoder):
        ...
        if encoder.rnn_state_shape:
            ...
            self.extract_rnn_state = lambda x: x[0] if x is not None else None
    ...
    def fwd_rollout(self, rnn_states_out, rnn_states_in, *obs_in):
        features = self._rollout_common(rnn_states_out, rnn_states_in, *obs_in)
    ...
    def fwd_sequence(self, rnn_start_states, sequence_breaks, *obs_in):
        ...
        features = self.encoder.fwd_sequence(
            self.extract_rnn_state(rnn_start_states), sequence_breaks, processed_obs
        )
...
class BackboneSeparate(Backbone):
    def __init__(self, process_obs, actor_encoder, critic_encoder):
        ...
        if actor_encoder.rnn_state_shape is None:
            ...
            self.extract_actor_rnn_state = lambda rnn_states: rnn_states[actor_rnn_idx]

        if critic_encoder.rnn_state_shape is None:
            ...
            self.extract_critic_rnn_state = lambda rnn_states: rnn_states[critic_rnn_idx]

        if actor_encoder.rnn_state_shape and critic_encoder.rnn_state_shape:
    ...
    def forward(self, rnn_states, *obs_in):
        ...
        return (
            actor_features,
            critic_features,
            self.package_rnn_states(new_actor_rnn_states, new_critic_rnn_states),
        )
    ...
    def _rollout_common(self, rnn_states_out, rnn_states_in, *obs_in):
        ...
        return self.encoder.fwd_inplace(rnn_states_out, rnn_states_in, processed_obs)
