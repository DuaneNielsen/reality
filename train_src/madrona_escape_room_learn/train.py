...
def _gather_minibatch(
    ...
    rnn_starts_slice = tuple(_mb_slice_rnn(state, inds) for state in rollouts.rnn_start_states)
...
def _compute_advantages(
    ...
    for i in reversed(range(cfg.steps_per_update)):
        ...
        cur_advantage = td_err + cfg.gamma * cfg.gae_lambda * next_valid * next_advantage
...
def _update_iter(
    ...
    with torch.no_grad():
        ...
        with profile("Compute Advantages"):
            _compute_advantages(cfg, amp, value_normalizer, advantages, rollouts)
    ...
    with profile("PPO"):
        ...
        for epoch in range(cfg.ppo.num_epochs):
            for inds in torch.randperm(num_train_seqs).chunk(cfg.ppo.num_mini_batches):
                with torch.no_grad(), profile("Gather Minibatch", gpu=True):
                    mb = _gather_minibatch(rollouts, advantages, inds, amp)
                cur_stats = _ppo_update(cfg, amp, mb, actor_critic, optimizer, value_normalizer)
...
def train(dev, sim, cfg, actor_critic, update_cb, restore_ckpt=None):
    ...
    value_normalizer = EMANormalizer(cfg.value_normalizer_decay, disable=not cfg.normalize_values)
