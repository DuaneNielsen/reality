...
def make_env(num_worlds=32, gpu_id=-1, transforms=True):
    ...
    env = MadronaEscapeRoomEnv(num_worlds=num_worlds, gpu_id=gpu_id, rand_seed=42, auto_reset=True)
        ...
        def forward(self, obs):
            ...
            if isinstance(obs, TensorDict):
                ...
                obs_concat = torch.cat(
                    [obs["self_obs"], obs["steps_remaining"], obs["agent_id"]], dim=-1
                )
            else:
                obs_concat = obs
    ...
    for i, batch in enumerate(collector):
        ...
        if rewards is not None:
            ...
            if len(episode_rewards) > 0:
                ...
                print(
                    f"Batch {i + 1}: Completed {len(episode_rewards)} episodes, "
