...
from madrona_escape_room import create_default_level


def create_complex_compiled_level():
    """
    Create a complex level with MANY obstacles to stress test physics/collisions.
    This uses the same layout as stress_test_complex_copy.py but creates a CompiledLevel.
    """
...
def main():
    ...
    print(f"Running {iterations} iterations with COMPLEX compiled level...")
    print("Testing: Complex collision detection with hundreds of obstacles")

    # Create complex level once
    complex_level = create_complex_compiled_level()
    ...
    for run in range(iterations):
        try:
            mgr = madrona_escape_room.SimManager(
                ...
                compiled_levels=[complex_level],  # Pass compiled level
            )
