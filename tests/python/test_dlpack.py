...
@pytest.mark.skipif(not hasattr(torch, 'from_dlpack'), reason="PyTorch version doesn't support DLPack")
...
def test_dlpack_multiple_tensors(cpu_manager):
    ...
    try:
        ...
        for madrona_tensor, expected_dtype, name in tensors_to_test:
            ...
            # Verify dtype
            assert torch_tensor.dtype == expected_dtype, f"{name} tensor should have dtype {expected_dtype}"
