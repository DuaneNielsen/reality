...
# Module-level check - skip all tests if PyTorch doesn't support DLPack
...
def require_dlpack_extension():
    ...
    try:
        # Try to call __dlpack__ to verify extension is loaded
        tensor.__dlpack__()
    except ImportError as e:
        if "DLPack extension module not found" in str(e):
            pytest.fail("DLPack extension not built - cannot test DLPack functionality")
        raise
