...
class TestLidarNoiseConfiguration:
    """Test lidar noise configuration in level compiler"""

    @pytest.mark.spec("docs/specs/level_compiler.md", "JSON Level Format (Single Level)")
    def test_lidar_noise_fields_single_level(self):
        """Test that lidar noise fields can be set via JSON single-level format"""
        json_level = {
            "ascii": ["########", "#S....#", "########"],
            "tileset": {
                "#": {"asset": "wall"},
            ...
            "scale": 2.5,
            "lidar_noise_factor": 0.01,  # 1% proportional noise
            "lidar_base_sigma": 0.02,  # 2cm base noise
        }

        compiled_levels = compile_level(json_level)
        level = compiled_levels[0]

        # Verify noise parameters were set correctly
        assert level.lidar_noise_factor == 0.01
        assert level.lidar_base_sigma == 0.02

    @pytest.mark.spec("docs/specs/level_compiler.md", "JSON Level Format (Single Level)")
    def test_lidar_noise_defaults_to_zero(self):
        """Test that lidar noise defaults to 0.0 (disabled) when not specified"""
        json_level = {
            "ascii": ["####", "#S.#", "####"],
            "tileset": {
                "#": {"asset": "wall"},
        ...
        # Should default to 0.0 (disabled)
        assert level.lidar_noise_factor == 0.0
        assert level.lidar_base_sigma == 0.0

    @pytest.mark.spec("docs/specs/level_compiler.md", "JSON Multi-Level Format")
    def test_lidar_noise_multi_level_shared(self):
        """Test lidar noise in multi-level format with shared configuration"""
        multi_level = {
            "levels": [
                {"ascii": ["####", "#S.#", "####"], "name": "level_1"},
                {"ascii": ["######", "#S..C#", "######"], "name": "level_2"},
            ...
            "lidar_noise_factor": 0.005,  # Shared noise config
            "lidar_base_sigma": 0.01,
        }

        compiled_levels = compile_multi_level(multi_level)

        # Both levels should inherit shared noise configuration
        assert len(compiled_levels) == 2
        for level in compiled_levels:
            assert level.lidar_noise_factor == 0.005
            assert level.lidar_base_sigma == 0.01

    @pytest.mark.spec("docs/specs/level_compiler.md", "JSON Multi-Level Format")
    def test_lidar_noise_multi_level_per_level_override(self):
        """Test that per-level noise config overrides shared config"""
        multi_level = {
            "levels": [
                {
                    "ascii": ["####", "#S.#", "####"],
                    "name": "level_1",
                    "lidar_noise_factor": 0.02,  # Override shared
                    "lidar_base_sigma": 0.05,
                },
                {"ascii": ["######", "#S..C#", "######"], "name": "level_2"},
            ],
            "tileset": {
            ...
            "lidar_noise_factor": 0.01,  # Shared default
            "lidar_base_sigma": 0.02,
        }

        compiled_levels = compile_multi_level(multi_level)

        # First level should use per-level override
        assert compiled_levels[0].lidar_noise_factor == 0.02
        assert compiled_levels[0].lidar_base_sigma == 0.05

        # Second level should use shared config
        assert compiled_levels[1].lidar_noise_factor == 0.01
        assert compiled_levels[1].lidar_base_sigma == 0.02

    @pytest.mark.spec("docs/specs/level_compiler.md", "JSON Level Format (Single Level)")
    def test_lidar_noise_validation_negative_values(self):
        """Test that negative lidar noise values are rejected"""
        # Test negative noise factor
        json_level = {
            "ascii": ["####", "#S.#", "####"],
            "tileset": {
                "#": {"asset": "wall"},
            ...
            "lidar_noise_factor": -0.01,  # Invalid
        }

        with pytest.raises(ValueError, match="Invalid lidar_noise_factor.*must be non-negative"):
            compile_level(json_level)

        # Test negative base sigma
        json_level["lidar_noise_factor"] = 0.01
        json_level["lidar_base_sigma"] = -0.02  # Invalid

        with pytest.raises(ValueError, match="Invalid lidar_base_sigma.*must be non-negative"):
            compile_level(json_level)

    @pytest.mark.spec("docs/specs/level_compiler.md", "JSON Level Format (Single Level)")
    def test_lidar_noise_validation_non_numeric(self):
        """Test that non-numeric lidar noise values are rejected"""
        # Test string noise factor
        json_level = {
            "ascii": ["####", "#S.#", "####"],
            "tileset": {
                "#": {"asset": "wall"},
