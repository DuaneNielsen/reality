...
from madrona_escape_room.default_level import create_default_levels
...
def test_cpp_vs_python_default_level_binary_identical():
    ...
    # Generate Python default levels to temporary file
    python_levels = create_default_levels()

    with tempfile.NamedTemporaryFile(suffix=".lvl", delete=False) as tmp:
        python_level_path = Path(tmp.name)

    try:
        # Save Python levels using unified format (now produces 2 levels like C++)
        save_compiled_levels(python_levels, python_level_path)
