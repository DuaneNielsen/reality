    ...
    def validate_tensor_consistency(mgr):
        ...
        for world_idx in range(steps.shape[0]):
            if done[world_idx, 0, 0].item() == 0:
                assert steps[world_idx, 0, 0].item() > 0, (
...
class TestBasicResetTiming:
    ...
    def test_RT_001_manual_reset_when_done_zero(self, cpu_manager):
        ...
        assert (steps_remaining[:, 0, 0] == expected_steps).all(), f"Expected {expected_steps} steps remaining"
        ...
        assert steps_remaining[0, 0, 0] == consts.episodeLen, "World 0 steps should be reset to episodeLen"
        
        # Other worlds should continue unaffected
        for world_idx in range(1, 4):
            expected_steps = consts.episodeLen - 51  # One more step
            assert steps_remaining[world_idx, 0, 0] == expected_steps, (
    ...
    def test_RT_003_auto_reset_off_done_one(self, cpu_manager):
        ...
        if len(done_worlds) > 0:
            ...
            assert done_tensor[done_world, 0, 0] == 1, "Done flag should persist with auto_reset=False"
...
class TestStepCounterPrecision:
    ...
    def test_SC_001_exact_episode_len_steps(self, cpu_manager):
        ...
        for step in range(consts.episodeLen):
            ...
            if step < consts.episodeLen - 1:
                ...
                assert (steps_remaining[:, 0, 0] == expected_remaining).all(), (
    ...
    def test_SC_003_reset_at_step_200_done_one(self, cpu_manager):
        ...
        if len(done_worlds) > 0:
            ...
            assert steps_remaining[done_world, 0, 0] == consts.episodeLen, "Should have full episode"
    ...
    def test_SC_004_multiple_resets_per_episode(self, cpu_manager):
        ...
        for reset_num in range(5):  # 5 resets
            ...
            assert steps_remaining[0, 0, 0] == consts.episodeLen, (
                f"Reset {reset_num+1}: World 0 should have full episode"
            )
            assert done_tensor[0, 0, 0] == 0, (
                f"Reset {reset_num+1}: World 0 should not be done"
            )
...
class TestCollisionVsStepTermination:
        ...
        for step in range(50):
            ...
            if done_tensor.any():
                ...
                for world_idx in done_worlds.flatten():
                    ...
                    assert done_tensor[world_idx, 0, 0] == 1, f"World {world_idx} should be done"
                    assert remaining_steps < consts.episodeLen, (
                        f"World {world_idx} terminated early with {remaining_steps} steps remaining"
                    )
                    
                    print(f"Collision detected at step {step+1} for world {world_idx}, "
    ...
    def test_CT_003_collision_plus_manual_reset(self, cpu_manager):
        ...
        if collision_world is not None:
            ...
            assert steps_remaining[collision_world, 0, 0] == consts.episodeLen, "Steps should be reset"
            
            print(f"✓ Manual reset successfully overrode collision termination for world {collision_world}")
        else:
...
class TestMultiWorldSynchronization:
    ...
    def test_MW_001_reset_world_0_others_continue(self, cpu_manager):
        ...
        assert (steps_remaining[:, 0, 0] == expected_steps).all(), "All worlds should be synchronized"
    ...
    def test_MW_002_multiple_worlds_done_simultaneously(self, cpu_manager):
        ...
        if len(done_worlds) >= 2:
            ...
            for world_idx in done_worlds[:2]:
                world_idx = world_idx.item()
                assert steps_remaining[world_idx, 0, 0] == consts.episodeLen, (
                    f"Reset world {world_idx} should have full episode"
                )
                assert done_tensor[world_idx, 0, 0] == 0, (
    ...
    def test_MW_003_auto_reset_with_mixed_done_states(self, cpu_manager):
        ...
        for world_idx in range(1, 4):
            assert abs(steps_remaining[world_idx, 0, 0].item() - expected_others) <= 1, (
    ...
    def test_MW_004_manual_reset_during_mixed_states(self, cpu_manager):
        ...
        for world_idx in range(4):
            assert 0 <= step_counts[world_idx] <= consts.episodeLen, (
                f"World {world_idx} has invalid step count: {step_counts[world_idx]}"
            )
            assert done_tensor[world_idx, 0, 0] in [0, 1], (
...
""")
    def test_OO_003_action_processing_during_reset(self, cpu_manager):
        ...
        assert current_steps == consts.episodeLen - 1, f"Steps should be {consts.episodeLen-1} due to execution order, got {current_steps}"
        ...
        assert distance_to_spawn < 1.0, f"Reset position should be close to spawn, distance: {distance_to_spawn}"
    ...
    def test_OO_004_observer_tensor_consistency(self, cpu_manager):
        ...
        for world_idx in [0, 2, 3]:
            ...
            if expected_steps <= 0:
                ...
                assert done_tensor[world_idx, 0, 0] == done_before[world_idx, 0, 0], (
...
class TestStatePersistence:
    ...
    def test_SP_002_steps_counter_across_reset(self, cpu_manager):
        ...
        for test_step in test_points:
            ...
            assert steps_remaining[0, 0, 0] == expected_remaining, (
    ...
    def test_SP_003_position_rotation_reset(self, cpu_manager):
        ...
        assert not torch.allclose(spawn_pos, current_pos, atol=0.1), (
        ...
        assert torch.allclose(spawn_pos, reset_pos, atol=0.1), (
        ...
        assert torch.allclose(spawn_rot, reset_rot, atol=0.2), (
    ...
    def test_SP_004_progress_reward_reset(self, cpu_manager):
        ...
        assert torch.allclose(initial_pos, reset_pos, atol=0.1), (
        ...
        print(f"Reset successfully returned agent to spawn")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if category == "basic":
            ...
            print("Usage: python test_comprehensive_reset_episode_counter_done_flag.py [basic|precision|collision|multiworld|order|persistence]")
    else:
