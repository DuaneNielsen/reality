...
def test_bidirectional_updates_actions(sim_manager):
    ...
    test_values = torch.tensor([[1, 2, 3], [2, 3, 4], [3, 4, 0], [0, 1, 2]], 
                              dtype=action_torch.dtype)
...
def test_bidirectional_updates_observations(sim_manager):
    ...
    assert (y_positions > initial_y).any(), \
        "Y positions didn't increase after forward movement"
...
def test_performance_characteristics(env):
    ...
    assert speedup > 1.2, \
        f"Zero-copy access not meaningfully faster ({speedup:.2f}x)"
    ...
    assert obs1.data_ptr() == obs2.data_ptr(), \
        "Repeated access returns different tensor instances"
...
def test_gpu_zero_copy(gpu_env):
    ...
    assert initial_ptr == final_ptr, \
        "GPU tensor data pointer changed after steps"
