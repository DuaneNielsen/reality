...
def test_rotation_in_place(test_manager):
    ...
    pos_delta = math.sqrt((final_pos[0] - initial_pos[0])**2 + (final_pos[1] - initial_pos[1])**2)
    ...
    assert abs(theta_delta) > 0.05, f"Agent should have rotated, but theta only changed by {theta_delta:.3f}"
