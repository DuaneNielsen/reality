#!/usr/bin/env python3
"""
Test agent strafe right movement.
"""
...
def test_strafe_right(test_manager):
    """Test agent strafing right (moving right while maintaining forward orientation)"""
    mgr = test_manager
    ...
    # Strafe right means moving 90 degrees to the right of facing direction
    strafe_angle = initial_theta + math.pi/2  # 90 degrees right
    ...
    if actual_length > 0.01:  # Avoid division by zero
        ...
        # Check movement direction (should be roughly 90 degrees right of facing)
        dot_product = actual_dx * expected_dx + actual_dy * expected_dy
        print(f"Movement direction alignment: {dot_product:.3f} (1.0 = perfect)")
        assert dot_product > 0.9, f"Agent should strafe right, but moved in wrong direction"
