...
def test_roundtrip_trajectory_file_verification(cpu_manager):
    ...
    try:
        ...
        assert len(record_lines) == len(replay_lines), f"Trace files should have same number of lines: record={len(record_lines)}, replay={len(replay_lines)}"
        ...
        for i, (record_line, replay_line) in enumerate(zip(record_lines, replay_lines)):
            assert record_line == replay_line, f"Trajectory mismatch at line {i+1}:\n  Record: {record_line}\n  Replay: {replay_line}"
        
        print(f"✓ Successfully verified {len(record_lines)} trajectory trace lines match exactly")
        print(f"✓ Record trace: {len(record_content)} chars, Replay trace: {len(replay_content)} chars")
...
def test_trajectory_file_verification_detects_differences(cpu_manager):
    ...
    try:
        ...
        assert trace1_content != trace2_content, "Different actions should produce different trajectory traces"
        ...
        assert len(trace1_lines) == len(trace2_lines), "Same number of steps should produce same number of trace lines"
        ...
        assert differences_found > 0, f"Expected trajectory differences but found none. Trace1 length: {len(trace1_content)}, Trace2 length: {len(trace2_content)}"
        
        print(f"✓ Successfully detected {differences_found} trajectory differences out of {len(trace1_lines)} lines")
        print(f"✓ This confirms our verification test would catch trajectory mismatches")
