...
# Size validation
assert CompiledLevel.size() == 84180, \
    f"CompiledLevel size mismatch: {CompiledLevel.size()} != 84180"

assert Action.size() == 12, \
    f"Action size mismatch: {Action.size()} != 12"

assert SelfObservation.size() == 20, \
    f"SelfObservation size mismatch: {SelfObservation.size()} != 20"

assert Done.size() == 4, \
    f"Done size mismatch: {Done.size()} != 4"

assert Reward.size() == 4, \
    f"Reward size mismatch: {Reward.size()} != 4"

assert Progress.size() == 4, \
    f"Progress size mismatch: {Progress.size()} != 4"

assert StepsRemaining.size() == 4, \
    f"StepsRemaining size mismatch: {StepsRemaining.size()} != 4"
