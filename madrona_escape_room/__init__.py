...
class SimManager:
    ...
    def __init__(
        ...
        # Use provided level or default level
        if level_ascii is None:
            # Default test level - 32x32 empty room for backward compatibility
            # Using JSON format to ensure level_name is properly set
            import json

            level_ascii = json.dumps(
                {
                    "name": "default_empty_room",
                    "scale": 2.5,
                    "ascii": "\n".join(
                        [
                            "################################",
        ...
        elif level_ascii.strip().startswith("{"):
            # JSON string format
            from .level_compiler import compile_level_from_json
